...
import TimezoneCombobox from './date-time/TimezoneCombobox';
import FormatPresetInput from './date-time/FormatPresetInput';

const TEXT_COMPARE_TOOL = 'text_compare';

interface CyberChefProps {
  initialCategory?: string;
  initialTool?: string;
  onStateChange?: (category: string, tool: string) => void;
}

export default function CyberChef({ initialCategory, initialTool, onStateChange }: CyberChefProps) {
  const categories = Array.from(new Set(allTools.map(t => t.category)));

  const resolvedCategory = (initialCategory && categories.includes(initialCategory))
    ? initialCategory : allTools[0].category;

  const toolsInResolved = allTools.filter(t => t.category === resolvedCategory);
  const resolvedTool = (initialTool && toolsInResolved.some(t => t.name === initialTool))
    ? initialTool : toolsInResolved[0]?.name ?? allTools[0].name;

  const [input, setInput] = useState('');
  const [output, setOutput] = useState('');
  const [selectedCategory, setSelectedCategory] = useState(resolvedCategory);
  const [selectedToolName, setSelectedToolName] = useState(resolvedTool);
  ...
  const handleCategoryChange = (cat: string) => {
    ...
    if (firstTool) {
        ...
        onStateChange?.(cat, firstTool.name);
        setParams({});
        setError(null);
    }
  };
  ...
  const handleToolChange = (name: string) => {
    ...
    onStateChange?.(selectedCategory, name);
    setParams({});
    setError(null);
  };
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const hasStartEnd = 'start' in shape && 'end' in shape;
      if (hasStartEnd && input.includes('\n')) {
        const lines = input.split('\n').map(s => s.trim()).filter(Boolean);
        if (lines.length >= 2) {
          typedArgs.start = params.start || lines[0];
          typedArgs.end = params.end || lines[1];
        }
      }
  ...
  const renderParams = () => {
    ...
    const TIMEZONE_KEYS = ['zone', 'fromZone', 'toZone'];
    const FORMAT_KEYS = ['inputFormat', 'outputFormat', 'format'];
    const hasDurationAmount = 'durationAmount' in shape;

    return Object.keys(shape).map(key => {
      if (key === 'input') return null;
      if (key === 'duration' && hasDurationAmount) return null;

      const fieldDef = shape[key];
      let required = true;
      let def = fieldDef;
      if (def instanceof z.ZodOptional) { required = false; def = def._def.innerType; }

      if (TIMEZONE_KEYS.includes(key)) {
        return <TimezoneCombobox key={key} label={key} required={required} value={params[key] || ''} onChange={v => setParams({ ...params, [key]: v })} />;
      }
      if (FORMAT_KEYS.includes(key)) {
        return <FormatPresetInput key={key} label={key} required={required} value={params[key] || ''} onChange={v => setParams({ ...params, [key]: v })} />;
      }
      ...
      if (typeName === 'ZodEnum') options = (def as any)._def.values;
      if (!typeName) {
        if (def instanceof z.ZodNumber) type = 'number';
        if (def instanceof z.ZodEnum) options = Object.values((def as any)._def.entries);
      }
  ...
  const isTextCompare = selectedToolName === TEXT_COMPARE_TOOL;

  return (
    <div className="flex flex-col h-full">
      {/* Category + Operation selector bar */}
      <div className="shrink-0 bg-slate-900 border-b border-slate-800 px-6 py-3 flex items-center gap-6">
        ...
        {toolsByCategory.length > 1 && (
          <select
            className="bg-slate-800 border border-slate-700 rounded px-2 py-1 text-slate-200 text-xs focus:ring-2 focus:ring-blue-500 outline-none"
      ...
      {/* Content area: swap layout based on tool */}
      {isTextCompare ? (
